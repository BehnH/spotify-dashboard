...
import {DividePath, sidenavIconClassname} from "@/components/PageFrame/utils";
        ...
        <nav className="flex flex-col items-center gap-4 px-2 sm:py-5">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Link
                href="/"
                className={sidenavIconClassname('/', window.location.pathname)}
              >
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Link
                href="/history"
                className={sidenavIconClassname('/history', window.location.pathname)}
              >
                <History className="h-5 w-5"/>
                <span className="sr-only">Play History</span>
              </Link>
            </TooltipTrigger>
            <TooltipContent side="right">Play History</TooltipContent>
          </Tooltip>
          <Separator />
          <Tooltip>
            <TooltipTrigger asChild>
              <Link
                href="/stats"
                className={sidenavIconClassname('/stats', window.location.pathname)}
              >
        ...
        <header className="sticky top-0 z-30 flex h-14 items-center gap-4 border-b bg-background px-4 sm:static sm:h-auto sm:border-0 sm:bg-transparent sm:px-6">
          ...
          <div className={"flex flex-row justify-between w-full"}>
            <Breadcrumb className="hidden md:flex">
              <BreadcrumbList>
                {DividePath(window.location.pathname).map((path, idx) => (
                  <Fragment key={idx}>
                    <BreadcrumbItem>
                      {path.isLast ? (
                    ...
                    {!path.isLast && <BreadcrumbSeparator />}
                  </Fragment>
                ))}
              </BreadcrumbList>
            </Breadcrumb>
